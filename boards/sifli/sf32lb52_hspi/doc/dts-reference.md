...
>
> **2026-07-02 更新**:DTS 已补全电池 GPADC(`&gpadc` + `channel@7` 内部 VBAT)、TF 卡
> (`sdhc0`/`sdmmc`,PA_29 改 GPIO 片选、PA_27 卡检测)、`zephyr,user`(PA_44 VBUS 检测 +
> "vbat" ADC 通道引用),并新增 AW32001 占位绑定;PA_01 经屏幕板网表证实 NC 悬空、无需建模。
> 本文相关小节已同步;修改明细见
> [architecture-and-dts-completeness.md §2.6](./architecture-and-dts-completeness.md)。
...
| `spi1_default` | PA_24(DIO), PA_25(DI), PA_28(CLK) | TF 卡 SPI;**PA_29 不在此组**,改由 `cs-gpios` 作 GPIO 片选 |
| `gpadc_default` | (空状态) | GPADC 仅用内部 VBAT 通道 7,外部 ADC 引脚全被占用 |
...
&spi1 {
    ...
    pinctrl-names = "default";
    cs-gpios = <&gpioa_00_31 29 GPIO_ACTIVE_LOW>;   // PA_29 GPIO 片选

    sdhc0: sdhc@0 {                                  // TF 卡(SPI 模式)
        compatible = "zephyr,sdhc-spi-slot";
        reg = <0>;
        spi-max-frequency = <24000000>;
        cd-gpios = <&gpioa_00_31 27 (GPIO_ACTIVE_LOW | GPIO_PULL_UP)>; // PA_27 SD_DET
        sdmmc: mmc {
...
| PA_28 | CLK |
| PA_29 | CS —— **GPIO 片选**(`cs-gpios`),不走 SPI1 硬件 CS:SD 协议要求跨多次传输保持 CS,`sdhc_spi` 依赖 `SPI_HOLD_ON_CS` |
| PA_27 | SD_DET 卡检测(上拉,低有效) |

TF 卡节点已建模(2026-07-02):应用侧启用 `CONFIG_SDHC`/`CONFIG_DISK_DRIVER_SDMMC` 后可经 disk API 或 fatfs 访问磁盘 "SD"。
